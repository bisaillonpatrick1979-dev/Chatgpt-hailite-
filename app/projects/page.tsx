...
const projects = [
  {
    name: "Chantier Hydro-Quebec",
    client: "Hydro-Quebec Renovations",
    tasks: ["Changer les fascias", "Changer le soffit", "Refaire le mur sud", "Photos finales"]
  },
  {
    name: "Riviere Rouge",
    client: "Residence privee",
    tasks: ["Enlever vieux siding", "Installer house wrap", "Installer siding", "Inspection admin"]
  },
  {
    name: "Leduc",
    client: "Bardage exterieur",
    tasks: ["Mur gauche", "Mur avant", "Trims", "Nettoyage"]
  }
];
  ...
  return (
    <AppShell>
      ...
      <section className="mt-5 grid gap-4 md:grid-cols-3">
        <ProMetric label="Projets actifs" value="3" note="Chantiers ouverts" tone="green" />
        <ProMetric label="Taches" value="12" note="A suivre par projet" tone="orange" />
            ...
            <article key={project.name} className="rounded-2xl border border-slate-700 bg-slate-950 p-5">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-2">
                {project.tasks.map((task) => (
                  <div key={task} className="rounded-xl border border-slate-800 bg-[#121821] p-4 text-xl font-black text-slate-100">
                    □ {task}
                  </div>
                ))}
              </div>
